...
<%
	...
	if (list == null) {
		...
		out.print("<script>location.href = '" + conPath + "/zhu'</script>");
	}
	%>
</head>
<body>
	...
	<%@ include file="/common/pre/searchBara.jsp"%>
	<!--End Header End-->
		...
		<div class="menu">
												...
												<div class="zj_l_c">
													...
													<c:forEach var="list3" items="${list3 }" varStatus="index2">
														<c:if test="${list3.parentId==list2.id }">
															<a
																href="${ctx}/queryProductList3?category=${list3.id}">${list3.name }</a>|
													</c:if>
													</c:forEach>
												</div>
											</c:if>
			...
			<ul class="menu_r">
				...
				<c:forEach items="${list}" var="temp">
					<li><a
			href="${ctx}/queryProductList?category=${temp.id}&menu=${temp.id}" <c:if test="${menu==temp.id}"> class="now" </c:if>>${temp.name}</a></li>
				</c:forEach>
			</ul>
	...
	<div class="i_bg bg_color">
			...
			<div class="banner">
				<div class="top_slide_wrap">
					<ul class="slide_box bxslider">
						<li><a href="${ctx}/queryProductDeatil?id=771">
						<img src="${ctx}/statics/images/a1.jpg" width="740"
							height="401" /></a></li>
						<li><a href="${ctx}/queryProductDeatil?id=772">
						<img src="${ctx}/statics/images/a2.jpg" width="740"
							height="401" /></a></li>
						<li><a href="${ctx}/queryProductDeatil?id=773">
						<img src="${ctx}/statics/images/a3.jpg" width="740"
							height="401" /></a></li>
						<li><a href="${ctx}/queryProductDeatil?id=774">						
		...
		<div class="content mar_10">
			<div class="h_l_img">
				<div class="img">
					<a href="${ctx}/queryProductDeatil?id=740">
									...
									<c:forEach var="productList" items="${productList }"
										...
										<li class="featureBox">
											<div class="box">
												...
												<div class="imgbg">
													<a
														href="${ctx}/queryProductDeatil?id=${productList.id}"><img
														src="${ctx}/files/${productList.fileName }" width="160"
														height="136" /></a>
												</div>
												<div class="name">
													<a
														href="${ctx}/queryProductDeatil?id=${productList.id}">
			...
			<div class="i_t mar_10">
				<span class="floor_num">${index.count }F</span> <span class="fl">${list.name }</span><span
					class="i_mores fr"> <c:forEach var="list2" items="${list2 }"
						...
						<c:if test="${list2.parentId==list.id }">
							<a
								href="${ctx}/queryProductList2?category=${list2.id}">${list2.name }</a>
						</c:if>
					</c:forEach>
				</span>
			</div>
						...
						<c:forEach var="productList" items="${productList }"
							...
							<c:if test="${productList.categoryLevel1==list.id }">
								<li>
									<div class="name">
										<a
											href="${ctx}/queryProductDeatil?id=${productList.id}">${productList.name }</a>
									</div>
									<div class="price">
										<font>￥<span>${productList.price }</span></font> &nbsp; 26R
									</div>
