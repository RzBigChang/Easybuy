				...
				$.ajax({
					...
					"success" : function(data) {
						...
						if (result.status == 1) {
							...
							$(".m_right").load(contextPath+"/category .m_right>*",ise);
						}
					}
				});
...
function toAddProductCategory() {
	$("#AddProductCategory").load(contextPath + "/AddProductCategory");
}
/**
 * 分类级别change事件！
 */
...
function queryProductCategoryList(onedd) {
	...
	$("#productCategoryLevel2")
			.load(
					contextPath
							+ "/addCategoryLevel2 #productCategoryLevel2>*",
					item);
}
...
function saveinsert() {
	...
	$.post(contextPath + "/insertCategory", item, function(data) {
		var result = eval("(" + data + ")");
		// 添加成功与否弹出信息！
		mizhu.alert('操作提醒', result.message);
		if (result.status == 1) {
			$(".m_right").load(contextPath+"/ac .m_right>*");
		}
...
function saveOrUpdate() {
	...
	$.post(contextPath + "/insertCategory", item, function(data) {
		...
		if (result.status == 1) {
			...
			$(".m_right").load(contextPath+"/ac .m_right>*",ise);
		}
...
function update(tid) {
	var item={
 //"action":"upProductCategory",
"id":tid};
	$("#AddProductCategory").load(contextPath + "/upProductCategory",item);
} 
