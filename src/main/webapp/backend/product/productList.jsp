...
<title>易买网</title>
	...
	<c:redirect url="${request.getContextPath()}/pre/Login.jsp"></c:redirect>
	</c:if>
	<%
		List list = (List) request.getAttribute("productList");
		if (list == null) {
			String conPath = request.getContextPath();
			out.print("<script>location.href = '" + conPath + "/spgl'</script>");
        ...
        <tr>
         ...
         <c:forEach items="${productList}" var="list">
          <tr>
            <td>${list.name}</td>
            <td>
              <a href="${ctx}/queryProductDeatil?id=${list.id}" target="_blank">
                  <img src="${ctx}/files/${list.fileName}" width="50" height="50"/>
              </a>
            </td>
            <td>${list.stock}</td>
            <td>${list.price}</td>
            <td><a href="javascript:;" onclick="updateByProductId('${list.id }','${pager.currentPage}')" return=false>修改</a></td>
